...
# Life satisfaction range filter
...
with tab_geo:
    ...
    # NEW: Average satisfaction bar chart by country (ranked)
    ...
    fig_bar.update_layout(
        ...
        plot_bgcolor="rgba(0,0,0,0)",
        font_family="DM Sans",
        yaxis_title="Avg. Life Satisfaction",
        xaxis_title="Country"
    )
...
with tab_drivers:
    ...
    with col_c:
        fig_eco = px.scatter(
            df, x="stfeco", y="stflife", trendline="ols",
    ...
    with col_d:
        fig_trust = px.scatter(
            df, x="social_trust", y="stflife", trendline="ols",
    ...
    if "age_group" in df.columns:
        ...
        fig_age = px.bar(
            age_sat, x="age_group", y="stflife",
            color="stflife", color_continuous_scale="Blues",
            text_auto=".2f",
            title="Average Life Satisfaction Across Age Groups"
        )
...
with tab_correlations:
    ...
    with col_f:
        st.subheader("Economy × Religiosity Interaction")
        if 'rel_bins' in df.columns and 'eco_bins' in df.columns:
            pivot_data = df.pivot_table(
                index="rel_bins", columns="eco_bins",
                values="stflife", aggfunc="mean"
            )
            fig_heat = px.imshow(
                pivot_data, text_auto=".1f",
    ...
    # NEW: Secular vs Non-Secular satisfaction violin plot
    if "secular_status" in df.columns:
        st.subheader("⛪ Life Satisfaction: Secular vs. Religious")
        fig_violin = px.violin(
            df, x="secular_status", y="stflife", color="secular_status",
...
# ── TAB 4: NEW Radar — Country Profiles ───────────────────────────────────────
with tab_radar:
    st.subheader("🕸️ Multi-Dimensional Country Profile Radar")
...
with st.expander("💡 Data Interpretation Guide"):
    ...
    - **Social Trust:** Composite score — how much respondents believe people are fair and helpful.
